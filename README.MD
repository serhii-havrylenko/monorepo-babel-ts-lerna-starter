...
`storybook` is configured, so it's time to create first story and test first package as well as `storybook` and `typescript` integration. Just create `LoginForm.story.tsx` inside `login-form` package.
